...
class Resource:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"uri": self.uri, "name": self.name, "mimeType": self.mimeType}
        if self.description:
    ...
    def read_resource_content(self, uri: str) -> str:
        ...
        if resource_type == "datasets":
            ...
            if result_type == "domains" or result_type == "all":
                ...
                if domain_keys:
                    results["domains"] = {
                        key: str(adata.uns[key]) for key in domain_keys[:3]
                    }  # Limit output
            ...
            if result_type == "communication" or result_type == "all":
                comm_keys = [
                    ...
                    if "communication" in key or "cellchat" in key
                ]
                if comm_keys:
            ...
            return json.dumps(
                {"message": "Visualization available through visualize_data tool"}
            )
