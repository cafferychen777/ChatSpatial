...
from .server import adapter, mcp
...
async def create_session():
    ...
    return JSONResponse(
        content={"session_id": session_id}, headers={"X-Session-Id": session_id}
    )
...
async def handle_rpc(request: MCPRequest, session_id: Optional[str] = None):
    ...
    try:
        ...
        return MCPResponse(error=error_response, id=request.id)
...
async def handle_initialize(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "serverInfo": {"name": "ChatSpatial", "version": "1.0.0"},
        "capabilities": {
...
def create_cli():
    ...
    parser = argparse.ArgumentParser(description="ChatSpatial MCP HTTP Server")
    parser.add_argument(
        "--host",
        default="127.0.0.1",
