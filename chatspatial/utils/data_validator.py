...
class DataValidationResult:
    ...
    def __init__(
        ...
        self.standardized_fields = (
            standardized_fields if standardized_fields is not None else {}
        )
...
class DataValidator:
    ...
    def _validate_basic_structure(
        self, adata: "ad.AnnData", result: DataValidationResult
    ) -> None:
    ...
    def _validate_spatial_coordinates(
        ...
        if spatial_key is None and found_alternatives:
            ...
            result.add_warning(
                f"Using non-standard spatial key '{spatial_key}', "
        ...
        if spatial_key is None:
            if "x" in adata.obs and "y" in adata.obs:
                result.add_warning(
                    "Found spatial coordinates in adata.obs['x'], adata.obs['y']. "
                    f"Should be in adata.obsm['{SPATIAL_KEY}']"
                )
                result.add_suggestion(
                    f"Move coordinates to adata.obsm['{SPATIAL_KEY}']"
                )
                return
        ...
        if spatial_key is None:
            ...
            result.add_suggestion(
                f"Add spatial coordinates to adata.obsm['{SPATIAL_KEY}']"
            )
            return
        ...
        if not isinstance(coords, self.np.ndarray):
            result.add_error(
                f"Spatial coordinates must be numpy array, got {type(coords)}"
            )
            return

        if coords.shape[0] != adata.n_obs:
            result.add_error(
                f"Spatial coordinates shape mismatch: "
                f"{coords.shape[0]} coordinates for {adata.n_obs} observations"
            )

        if coords.shape[1] < 2:
            result.add_error(
                f"Spatial coordinates must have at least 2 dimensions (x, y), "
        ...
        if coords.dtype != "float64":
            result.add_warning(
                f"Spatial coordinates should be float64, " f"found {coords.dtype}"
            )
    ...
    def _validate_expression_matrix(
        ...
        if adata.X.shape != expected_shape:
            result.add_error(
                f"Expression matrix shape mismatch: "
        ...
        if self.np.any(data < 0):
            ...
            result.add_suggestion(
                "Consider using raw counts or properly normalized data"
            )
    ...
    def _validate_gene_names(
        ...
        ensembl_pattern = any(
            name.startswith(("ENSG", "ENSMUSG")) for name in gene_names[:100]
        )
        if ensembl_pattern:
            result.add_warning("Gene names appear to be Ensembl IDs")
            result.add_suggestion(
                "Consider converting to gene symbols for better compatibility"
            )

        # Check for whitespace issues
        whitespace_issues = [
            name
            for name in gene_names[:100]
            if isinstance(name, str) and (name != name.strip())
        ]
        if whitespace_issues:
            result.add_warning(
                f"Found gene names with whitespace issues: {whitespace_issues[:5]}"
            )
    ...
    def _validate_optional_fields(
        ...
        if cell_type_found:
            ...
            if key != standard_key:
                ...
                result.add_warning(
                    f"Found cell types in non-standard field '{key}', "
                    f"should be '{standard_key}'"
                )
            self._validate_categorical_field(
                adata.obs[key], f"Cell types ({key})", result
            )
        ...
        if cluster_found:
            ...
            if key != standard_key:
                result.standardized_fields[key] = standard_key
                result.add_warning(
                    f"Found clusters in non-standard field '{key}', "
                    f"should be '{standard_key}'"
                )
            self._validate_categorical_field(
                adata.obs[key], f"Clusters ({key})", result
            )
        ...
        if batch_found:
            ...
            if key != standard_key:
                result.standardized_fields[key] = standard_key
                result.add_warning(
                    f"Found batch info in non-standard field '{key}', "
                    f"should be '{standard_key}'"
                )
            self._validate_categorical_field(
                adata.obs[key], f"Batch info ({key})", result
            )
    ...
    def _validate_cell_types(
        ...
        if not cell_type_found:
            ...
            result.add_suggestion(
                f"Add cell type annotations to adata.obs['{CELL_TYPE_KEY}']"
            )
            return
        ...
        if cell_types.isna().any():
            ...
            result.add_error(
                f"Cell type annotations contain {n_missing} missing values"
            )
        ...
        if len(unique_types) < 2:
            ...
            result.add_suggestion(
                "Consider if cell type diversity is sufficient for analysis"
            )

    def _validate_analysis_requirements(
        self, adata: "ad.AnnData", analysis_type: str, result: DataValidationResult
    ) -> None:
        ...
        if analysis_type.lower() == "cell_communication":
            ...
            if not self._find_field_in_obs(adata, ALTERNATIVE_CELL_TYPE_KEYS):
                result.add_error(
                    "Cell communication analysis requires cell type annotations"
                )

            if "spatial_connectivities" not in adata.obsp:
                result.add_warning("Spatial connectivity graph not found")
                result.add_suggestion(
                    "Run sq.gr.spatial_neighbors(adata) to compute spatial graph"
                )
            ...
            if not self._find_field_in_obs(adata, ALTERNATIVE_CLUSTER_KEYS):
                ...
                result.add_suggestion(
                    f"Run clustering and add results to adata.obs['{CLUSTER_KEY}']"
                )

    def _validate_categorical_field(
        self, series: "pd.Series", field_name: str, result: DataValidationResult
    ) -> None:
        ...
        if not self.pd.api.types.is_categorical_dtype(series):
            ...
            result.add_suggestion(
                "Convert to categorical: adata.obs[field] = adata.obs[field].astype('category')"
            )
        ...
        if n_categories > 100:
            ...
            result.add_suggestion(
                "Consider if this categorical variable is appropriate"
            )

    def _find_field_in_obs(
        self, adata: "ad.AnnData", alternatives: Set[str]
    ) -> Optional[tuple]:
        """
    ...
    def _suggest_standardization(
        ...
        if "spatial_connectivities" not in adata.obsp and any(
            ...
            result.add_suggestion(
                "Compute spatial neighbors: sq.gr.spatial_neighbors(adata)"
            )

        # Check if highly variable genes are identified
        if "highly_variable" not in adata.var.columns:
            result.add_suggestion(
                "Identify highly variable genes: self.sc.pp.highly_variable_genes(adata)"
            )
...
def validate_spatial_data(
    ...
    return validator.validate_adata(
        adata, require_spatial=True, require_cell_types=False
    )


def validate_for_cell_communication(
    adata: "ad.AnnData", strict: bool = False
) -> DataValidationResult:
...
def validate_for_deconvolution(
    adata: "ad.AnnData",
    ...
    if reference_adata is not None:
        ref_result = validator.validate_adata(
            reference_adata, require_spatial=False, require_cell_types=True
        )

        # Merge results
        result.errors.extend([f"Reference data: {err}" for err in ref_result.errors])
        result.warnings.extend(
            [f"Reference data: {warn}" for warn in ref_result.warnings]
        )
...
def raise_if_invalid(
    adata: "ad.AnnData",
    analysis_type: Optional[str] = None,
    message_prefix: str = "Data validation failed",
) -> None:
    """
