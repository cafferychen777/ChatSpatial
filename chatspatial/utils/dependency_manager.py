...
class DependencyManager:
    ...
    def check_dependency(
        ...
        if dep_info.python_version_constraint:
            if not self._check_python_constraint(dep_info.python_version_constraint):
                error_msg = (
                    f"{dep_name} requires Python {dep_info.python_version_constraint}, "
    ...
    def require_dependency(self, dep_name: str, feature: str = None) -> Any:
        ...
        if not available:
            dep_info = self._get_dependency_info(dep_name)
            install_cmd = (
                dep_info.install_command if dep_info else f"pip install {dep_name}"
            )
    ...
    def try_import(
        self, dep_name: str, feature: str = None
        ...
        if not available:
            ...
            feature = feature or dep_name
            warning = (
                f"Optional dependency '{dep_name}' not available for {feature}. "
