...
class SpatialFallbacks:
    ...
    @staticmethod
    def spatial_neighbors_fallback(
        adata: sc.AnnData, n_neighbors: int = 6, spatial_key: str = "spatial", **kwargs
    ) -> None:
        """
    ...
    @staticmethod
    def spatial_autocorrelation_fallback(
        adata: sc.AnnData, genes: Optional[List[str]] = None, **kwargs
    ) -> pd.DataFrame:
        """
...
class CellCommunicationFallbacks:
    ...
    @staticmethod
    def simple_ligand_receptor_analysis(
        adata: sc.AnnData, cell_type_key: str = "cell_type", **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "n_significant": (
                len(df[df["communication_score"] > df["communication_score"].median()])
                if len(df) > 0
                else 0
            ),
        }
...
class SpatialDomainFallbacks:
    ...
    def kmeans_spatial_domains(
        ...
        **kwargs,
        ...
        # Store results
        adata.obs["spatial_domains"] = pd.Categorical([f"Domain_{i}" for i in clusters])
    ...
    @staticmethod
    def expression_based_domains(
        adata: sc.AnnData, resolution: float = 0.5, **kwargs
    ) -> None:
        """
        Expression-only spatial domain identification using Leiden clustering.
        """
        warn_fallback(
            "spatial domains", "SpaGCN/STAGATE", "expression-based Leiden clustering"
        )
...
class DeconvolutionFallbacks:
    ...
    def nnls_deconvolution(
        ...
        **kwargs,
        ...
        # Get common genes
        common_genes = list(
            set(adata_spatial.var_names) & set(adata_reference.var_names)
        )
        if len(common_genes) < 100:
            raise ValueError(
                f"Too few common genes ({len(common_genes)}). Need at least 100."
            )
        ...
        return {
            "proportions": pd.DataFrame(
                prop_matrix, columns=cell_types, index=adata_spatial.obs_names
            ),
...
class VisualizationFallbacks:
    ...
    def simple_spatial_plot(
        ...
        **kwargs,
        ...
        """
        warn_fallback(
            "spatial visualization", "advanced plotting", "matplotlib scatter plot"
        )
            ...
            if color in adata.obs.columns:
                ...
                if color_data.dtype == "category" or color_data.dtype == "object":
                    ...
                    for i, category in enumerate(
                        color_data.cat.categories
                        if hasattr(color_data, "cat")
                        else color_data.unique()
                    ):
                ...
                scatter = ax.scatter(
                    coords[:, 0], coords[:, 1], c=expr, s=20, alpha=0.7, cmap="viridis"
                )
                plt.colorbar(scatter, ax=ax)
...
class SafeMode:
    ...
    def basic_visualization(self) -> None:
        ...
        if "spatial" in self.adata.obsm:
            fig = VisualizationFallbacks.simple_spatial_plot(self.adata, color="leiden")
