...
def fig_to_image(
    ...
    current_dpi = dpi
    min_dpi = 50  # Don't go below this to maintain readability
    
    for attempt in range(3):  # Try up to 3 times with decreasing DPI
        buf = io.BytesIO()
        ...
        try:
            ...
            if size_kb <= max_size_kb or current_dpi <= min_dpi:
                if close_fig:
                    plt.close(fig)
                # Return Image object with bytes data
                return Image(data=img_data, format=format)
            
            # Otherwise, reduce DPI and try again
            current_dpi = max(min_dpi, int(current_dpi * 0.7))  # Reduce DPI by 30%
            
        except Exception as e:
            print(f"Error saving figure: {str(e)}")
            # Try with a lower DPI
            current_dpi = max(min_dpi, int(current_dpi * 0.7))
    ...
    simple_fig = plt.figure(figsize=(4, 4), dpi=72)
    ...
    buf = io.BytesIO()
    simple_fig.savefig(buf, format=format, dpi=72, bbox_inches='tight')
