...
class DependencySpec:
    ...
    platform_specific: Dict[str, str] = field(default_factory=dict)  # OS-specific install
...
class SmartImporter:
    ...
    def _build_dependency_registry(self) -> Dict[str, DependencySpec]:
        ...
        core_deps = [
            DependencySpec("numpy", "numpy", DependencyLevel.CORE, 
                          "Core numerical computing"),
            DependencySpec("pandas", "pandas", DependencyLevel.CORE,
                          "Data manipulation and analysis"),
            DependencySpec("scanpy", "scanpy", DependencyLevel.CORE,
                          "Single-cell analysis toolkit"),
            DependencySpec("anndata", "anndata", DependencyLevel.CORE,
                          "Annotated data matrices"),
            DependencySpec("matplotlib", "matplotlib", DependencyLevel.CORE,
                          "Basic plotting functionality"),
            DependencySpec("scipy", "scipy", DependencyLevel.CORE,
                          "Scientific computing"),
            DependencySpec("sklearn", "scikit-learn", DependencyLevel.CORE,
                          "Machine learning toolkit", pip_name="scikit-learn"),
        ]
        
        # RECOMMENDED dependencies - Most users need these
        recommended_deps = [
            DependencySpec("squidpy", "squidpy", DependencyLevel.RECOMMENDED,
                          "Spatial transcriptomics analysis toolkit"),
            DependencySpec("umap", "umap-learn", DependencyLevel.RECOMMENDED,
                          "Dimensionality reduction and visualization"),
            DependencySpec("igraph", "python-igraph", DependencyLevel.RECOMMENDED,
                          "Graph analysis and network algorithms"),
            DependencySpec("leidenalg", "leidenalg", DependencyLevel.RECOMMENDED,
                          "Community detection clustering"),
            DependencySpec("harmonypy", "harmonypy", DependencyLevel.RECOMMENDED,
                          "Batch effect correction"),
            DependencySpec("scvelo", "scvelo", DependencyLevel.RECOMMENDED,
        ...
        advanced_deps = [
            ...
            DependencySpec("cell2location", "cell2location", DependencyLevel.ADVANCED,
                          "Spatial deconvolution with cell2location",
                          install_cmd="pip install cell2location"),
            
            # Spatial domain identification  
            DependencySpec("SpaGCN", "SpaGCN", DependencyLevel.ADVANCED,
            ...
            DependencySpec("STAGATE", "STAGATE", DependencyLevel.ADVANCED,
                          ...
                          install_cmd="pip install STAGATE"),
            DependencySpec("banksy", "banksy-python", DependencyLevel.ADVANCED,
                          "Neighborhood aggregation for spatial domains",
                          install_cmd="pip install banksy-python"),
        ...
        experimental_deps = [
            DependencySpec("enrichmap", "enrichmap", DependencyLevel.EXPERIMENTAL,
                          "Enhanced pathway enrichment visualization",
                          install_cmd="pip install enrichmap"),
            DependencySpec("petsc4py", "petsc4py", DependencyLevel.EXPERIMENTAL,
                          "High-performance linear algebra for CellRank",
                          install_cmd="pip install petsc4py"),
            DependencySpec("slepc4py", "slepc4py", DependencyLevel.EXPERIMENTAL,
    ...
    def smart_import(self, package_name: str, 
                    fallback_name: Optional[str] = None,
                    required: bool = True) -> ImportResult:
        """
        ...
        if not result.success:
            result = self._enhance_error_with_suggestions(package_name, result, required)
    ...
    def _enhance_error_with_suggestions(self, package_name: str, 
                                      result: ImportResult, 
                                      required: bool) -> ImportResult:
        ...
        if not dep_spec:
            ...
            result.suggestion = f"Package '{package_name}' not found. Try: pip install {package_name}"
            return result
        ...
        if dep_spec.level == DependencyLevel.CORE:
            suggestion_parts.append("⚠️  This is required for ChatSpatial to function properly.")
        ...
        if package_name == "torch":
            suggestion_parts.append("Note: PyTorch installation may require platform-specific commands.")
        elif package_name == "tensorflow":
            suggestion_parts.append("Note: Using TensorFlow 1.x for STAGATE compatibility.")
    ...
    def get_available_features(self) -> Dict[str, List[str]]:
        ...
        if self.smart_import("squidpy").success:
            features["spatial_analysis"].extend(["spatial_neighbors", "spatial_autocorr", "co_occurrence"])
    ...
    def install_dependencies(self, level: Union[DependencyLevel, str], 
        ...
        results = {}
        packages_to_install = [
            spec for spec in self._dependency_registry.values()
            if spec.level == level
        ]
        
        if dry_run:
            print(f"Would install {len(packages_to_install)} {level.value} dependencies:")
        ...
        for spec in packages_to_install:
            try:
                ...
                subprocess.run(spec.install_cmd.split(), 
...
def smart_import(package_name: str, 
                fallback_name: Optional[str] = None,
                required: bool = True) -> ImportResult:
    """
...
def graceful_import(package_name: str, 
        ...
        async def wrapper(*args, **kwargs):
            ...
            if result.success:
                ...
                if fallback_func:
                    ...
                    if context:
                        await context.warning(f"Using fallback for {func.__name__}: {result.suggestion}")
                    return await fallback_func(*args, **kwargs)
                else:
                    raise ImportError(f"Missing dependency for {func.__name__}: {result.suggestion}")
