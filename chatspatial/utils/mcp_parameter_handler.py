...
def validate_parameters_manually(
    ...
    if params_dict is None:
        try:
            ...
            raise ValueError(
                f"Default parameter validation failed: {format_pydantic_errors(e.errors())}"
            )
...
def format_pydantic_errors(errors: list) -> str:
    ...
    for error in errors:
        field_path = (
            ".".join(str(x) for x in error["loc"]) if error["loc"] else "root field"
        )
        ...
        if error_type == "greater_than":
            ...
            message = (
                f"  • {field_path}: must be less than {max_value}, got: {error_input}"
            )
...
def validate_visualization_params(params: Union[Dict[str, Any], BaseModel, None]):
    ...
    return validate_parameters_manually(
        preprocessed_params, VisualizationParameters, "visualization_params"
    )


def _preprocess_visualization_params(
    params: Union[Dict[str, Any], BaseModel, str, None],
) -> Union[Dict[str, Any], BaseModel, None]:
    """
...
def validate_cell_communication_params(params: Union[Dict[str, Any], BaseModel, None]):
    ...
    return validate_parameters_manually(
        params, CellCommunicationParameters, "cell_communication_params"
    )
...
def validate_annotation_params(params: Union[Dict[str, Any], BaseModel, None]):
    ...
    return validate_parameters_manually(
        params, AnnotationParameters, "annotation_params"
    )
