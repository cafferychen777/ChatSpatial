...
from .smart_import import get_smart_importer, DependencyLevel
...
class UserGuidanceSystem:
    ...
    def _build_installation_guides(self) -> Dict[str, InstallationGuide]:
        ...
        guides["cellphonedb"] = InstallationGuide(
            ...
            methods={
                InstallationMethod.PIP: "pip install cellphonedb"
            },
        ...
        guides["cell2location"] = InstallationGuide(
            ...
            methods={
                InstallationMethod.PIP: "pip install cell2location"
            },
            prerequisites=["torch", "scanpy", "anndata"],
            platform_notes={
                "all": "Requires PyTorch - large download (~1GB)"
            },
            troubleshooting=[
        ...
        guides["SpaGCN"] = InstallationGuide(
            ...
            methods={
                InstallationMethod.PIP: "pip install SpaGCN"
            },
        ...
        guides["STAGATE"] = InstallationGuide(
            ...
            methods={
                InstallationMethod.PIP: "pip install STAGATE"
            },
        ...
        guides["rpy2"] = InstallationGuide(
            ...
            methods={
                InstallationMethod.PIP: "pip install rpy2"
            },
...
"""
    ...
    def check_system_health(self) -> Dict[str, Any]:
        ...
        if self.python_version < (3, 8):
            health_report["issues"].append(f"Python {self.python_version.major}.{self.python_version.minor} is too old")
            health_report["recommendations"].append("Upgrade to Python 3.8+")
        elif self.python_version >= (3, 12):
            health_report["issues"].append("Python 3.12+ may have compatibility issues with some packages")
            health_report["recommendations"].append("Consider using Python 3.10 or 3.11 for maximum compatibility")
        ...
        if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
            ...
            health_report["recommendations"].append("Consider using a virtual environment")
        
        # Platform-specific checks
        if self.platform == "windows":
            health_report["recommendations"].append("Consider installing Microsoft Visual C++ Build Tools for complex packages")
        elif self.platform == "darwin" and platform.machine() == "arm64":
            health_report["recommendations"].append("Ensure packages support Apple Silicon")
        
        return health_report
    
    def generate_installation_script(self, level: DependencyLevel, 
                                   method: InstallationMethod = InstallationMethod.PIP) -> str:
        """Generate installation script for a dependency level"""
        
        importer = get_smart_importer()
        packages_by_level = {
            dep.name: dep for dep in importer._dependency_registry.values()
            if dep.level == level
        }
    ...
    def create_requirements_file(self, level: DependencyLevel, 
                               output_file: Optional[str] = None) -> str:
        """Create requirements.txt file for a dependency level"""
        
        importer = get_smart_importer()
        packages = [
            dep.pip_name for dep in importer._dependency_registry.values()
            if dep.level == level
        ]
