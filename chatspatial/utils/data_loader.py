            ...
            if os.path.isdir(data_path):
                ...
                if spatial_path:
                    try:
                        ...
                        logger.info(f"Successfully added spatial information from {spatial_path}")
                    except Exception as e:
                        logger.warning(f"Could not add spatial information: {str(e)}")
                        logger.info("Proceeding with expression data only")
                else:
                    logger.info("No spatial folder found. Loading expression data only.")
                    logger.info("Tip: Place spatial files in a 'spatial' folder in the same directory as the H5 file")
                ...
                if 'spatial' not in adata.uns and not any('spatial' in key for key in adata.obsm.keys()):
                    logger.warning("The h5ad file does not contain spatial information typically required for 10x Visium data")
            else:
                raise ValueError(f"Unsupported file format for 10x_visium: {data_path}. Supported formats: directory with Visium structure, .h5 file, or .h5ad file")
...
def _find_spatial_folder(h5_path: str) -> Optional[str]:
    ...
    candidates = [
        ...
        os.path.join(base_dir, base_name.replace('_filtered_feature_bc_matrix', '_spatial')),
...
def _add_spatial_info_to_adata(adata: Any, spatial_path: str) -> Any:
    ...
    try:
        ...
        if len(common_barcodes) == 0:
            ...
            elif all('-1' not in bc for bc in adata.obs_names[:10]) and all('-1' in bc for bc in positions.index[:10]):
        ...
        logger.info(f"Found {len(common_barcodes)} matching barcodes out of {len(adata)} cells")
        ...
        adata.obsm['spatial'] = positions[['pxl_col_in_fullres', 'pxl_row_in_fullres']].values.astype(float)
