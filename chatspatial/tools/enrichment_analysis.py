...
# Visualization wrapper functions
async def visualize_enrichment(
    data_id: str,
    data_store: Dict[str, Any],
    score_key: str,
    ...
    **kwargs
    ...
    """
    Visualize enrichment scores using EnrichMap plotting functions.
    ...
    score_key : str
        Column name containing the enrichment scores
    plot_type : str
        Type of plot: 'spatial', 'violin', 'correlogram', 'variogram'
    ...
    **kwargs
    ...
    try:
        ...
        if plot_type == "spatial":
            em.pl.spatial_enrichmap(adata, score_key=score_key, **kwargs)
        elif plot_type == "violin":
            if "cluster_key" not in kwargs:
                kwargs["cluster_key"] = "leiden"
            em.pl.spatial_metrics(adata, score_key=score_key, **kwargs)
        elif plot_type == "correlogram":
            em.pl.morans_correlogram(adata, score_key=score_key, **kwargs)
        elif plot_type == "variogram":
            em.pl.variogram(adata, score_key=score_key, **kwargs)
        else:
        ...
        raise ToolError(f"Visualization failed: {str(e)}")