...
SCENARIOS = [
    ...
    (
        ...
        "Standard Visium Input",
        "Visium, 4992 spots, h5ad",
        "Load and preprocess the Visium data.",
        "Data loaded, QC metrics calculated.",
        "Pass",
        "Standard workflow.",
    ),
    (
        2,
        "Data Handling & Preprocessing",
        "High-res Xenium Input",
        "Xenium, 150k cells",
        "Load Xenium data and create AnnData.",
        "Data loaded, valid AnnData created.",
        "Pass",
        "Non-h5ad input.",
    ),
    (
        ...
        "Data Handling & Preprocessing",
        "Low-quality Data",
        "Visium, <500 genes/spot",
        "Filter low-quality spots.",
        "Low-quality spots removed.",
        "Pass",
        "QC validation.",
    ),
    (
    ...
    (
        ...
        "Basic Clustering",
        "DLPFC Visium (spatialLIBD)",
        "Identify spatial domains using SpaGCN.",
        "Anatomical layers identified.",
        "Pass",
        "Core function.",
    ),
    (
    ...
    (
        ...
        "Mouse brain",
        "Find top 50 SVGs with SPARK-X.",
        "Known markers ranked high.",
        "Pass",
        "SVG detection.",
    ),
    (
        ...
        "Cell Deconvolution",
        "Visium + scRNA ref",
        "Deconvolve cell types using RCTD.",
        "Cell proportions generated.",
        "Pass",
        "Deconvolution.",
    ),
    (
        ...
        "Cell Annotation",
        "Xenium + atlas",
        "Annotate cells with Tangram.",
        "Major types assigned.",
        "Pass",
        "Annotation test.",
    ),
    (
        ...
        "Trajectory Analysis",
        "Dev. tissue data",
        "Infer trajectory using CellRank.",
    ...
    (
        ...
        "Core Spatial Analysis",
        "RNA Velocity",
        "Visium + spliced counts",
        "Calculate spatial RNA velocity.",
        "Dynamics visualized.",
        "Pass",
        "Complex analysis.",
    ),
    (
        ...
        "Spatial Statistics",
        "Mouse brain Visium",
        "Compute Moran's I for top marker genes.",
        "Spatial autocorrelation scores generated.",
        "Pass",
        "Statistics test.",
    ),
    (
        ...
        "Pathway Enrichment",
        "Cancer Visium",
        "Run GSEA on cluster markers using GO database.",
        "Enriched pathways identified.",
        "Pass",
        "Enrichment test.",
    ),
    (
        ...
        "CNV Detection",
        "Tumor Visium + ref",
        "Infer CNV using infercnvpy with immune cells as reference.",
        "CNV profiles generated.",
        "Pass",
        "CNV test.",
    ),
    (
        ...
        "Serial sections",
        "Align 3 consecutive slices with PASTE.",
        "Slices registered to common space.",
        "Pass",
        "Registration test.",
    ),
    (
        ...
        "Iterative Analysis",
        "Tumor microenv.",
        "Find clusters. Now, markers for cluster 3.",
        "Context preserved.",
        "Pass",
        "Context test.",
    ),
    ...
    (
        ...
        "Parameter Override",
        "Visium data",
        "Run STAGATE with resolution 1.5.",
        "User params used.",
        "Pass",
        "User control.",
    ),
    (
        ...
        "Cross-category Flow",
        "Cancer + ref",
        "Cluster, then find tumor-immune comm.",
    ...
    (
        ...
        "Concurrent Users",
        "2 users, diff. data",
        "U1: Cluster brain. U2: Annotate tumor.",
        "No interference.",
        "Pass",
        "Isolation test.",
    ),
    ...
    (
        ...
        "Invalid Input",
        "Non-scientific cmd",
        "Write Python code to download a file from the internet.",
        "Polite refusal; redirected to spatial analysis scope.",
        "Pass",
        "Scope boundary test.",
    ),
    (
        ...
        "Unsupported method",
        "Visium data",
        "Run BayesSpace for spatial domains.",
        "Tool invoked but method not in enum; error returned listing available methods.",
        "Partial",
        "Method outside curated set.",
    ),
    (
        30,
        "Known Limitations",
        ...
        "Tumor Visium",
        "Which subclone is most likely to metastasize?",
        "System identified relevant analyses but could not synthesize a biological conclusion; deferred to researcher.",
        "Partial",
        "Requires domain judgment beyond orchestration.",
    ),
    (
        31,
        "Known Limitations",
        "Very large Visium HD",
        "500k+ spots",
        "Run SpaGCN spatial domains.",
        "SpaGCN timed out (600s limit); system suggested Leiden as scalable alternative.",
        "Partial",
        "Algorithm-level scalability limit, not framework limit.",
    ),
]
...
def write_effect_sizes():
    ...
    if not tests_path.exists() or not effects_path.exists():
        if path.exists() and path.stat().st_size > 0:
            print(
                f"  S1: {path.name} (existing committed table; source Case2_OSCC files not present)"
            )
            return
        ...
        rows.append(
            {
                ...
                "Median log2FC 95% CI lower": (
                    round(ci_lo, 2) if pd.notna(ci_lo) else ""
                ),
                "Median log2FC 95% CI upper": (
                    round(ci_hi, 2) if pd.notna(ci_hi) else ""
                ),
                "Sign test (positive / negative)": f"{int(r['sign_n_pos'])}/{int(r['sign_n_neg'])}",
                "Sign test p": round(r["sign_p"], 4) if pd.notna(r["sign_p"]) else "",
                "Mean |r| (all samples)": (
                    round(mean_r_all, 3) if pd.notna(mean_r_all) else ""
                ),
                "n (all)": int(r["n_all"]),
                "Mean |r| (significant samples)": (
                    round(mean_r_sig, 3) if pd.notna(mean_r_sig) else ""
                ),
...
def write_methods():
    ...
    with open(path, "w", newline="") as f:
        ...
        w.writerow(
            ["Analytical Category", "Method Name", "Version", "Primary Citation"]
        )
        for row in METHODS:
            w.writerow(row)
