...
def postprocess_sample(export_dir: Path, sample_id: str):
    ...
    adata.obs = (
        adata.obs[keep_cols] if keep_cols else pd.DataFrame(index=adata.obs_names)
    )
    ...
    if "spatial" not in adata.obsm:
        ...
        if "array_row" in adata.obs.columns and "array_col" in adata.obs.columns:
            adata.obsm["spatial"] = adata.obs[["array_col", "array_row"]].values.astype(
                float
            )
...
def main():
    ...
    if len(existing) == len(TARGET_SAMPLES):
        ...
        for sid in TARGET_SAMPLES:
            ...
            print(
                f"  {sid}: {adata.shape[0]} spots x {adata.shape[1]} genes, "
