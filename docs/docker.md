...
This guide uses the versioned `v1.5.0` tag so commands are reproducible. Use
...
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.5.0 --version
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.5.0 server --help
```
...
docker run --rm -p 127.0.0.1:8000:8000 \
  -v /absolute/path/to/your/data:/data:ro \
  -v /absolute/path/to/outputs:/outputs \
  ghcr.io/cafferychen777/chatspatial:v1.5.0 server \
...
Then replace `ghcr.io/cafferychen777/chatspatial:v1.5.0` with
...
| Pull fails | Check the image name and network access with `docker pull ghcr.io/cafferychen777/chatspatial:v1.5.0`. |
