...
This guide uses the versioned `v1.2.9` tag so commands are reproducible. Use
...
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.2.9 --version
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.2.9 server --help
```
...
docker run --rm -p 8000:8000 \
  -v /absolute/path/to/your/data:/data:ro \
  -v /absolute/path/to/outputs:/outputs \
  ghcr.io/cafferychen777/chatspatial:v1.2.9 server --transport sse --host 0.0.0.0 --port 8000
```
...
Then replace `ghcr.io/cafferychen777/chatspatial:v1.2.9` with
...
| Pull fails | Check the image name and network access with `docker pull ghcr.io/cafferychen777/chatspatial:v1.2.9`. |
