...
```text
ghcr.io/cafferychen777/chatspatial:v1.2.8
```

This guide uses the versioned `v1.2.8` tag so commands are reproducible. Use
`:latest` only when you explicitly want the newest published image.

## Pull and verify the image

```bash
docker pull ghcr.io/cafferychen777/chatspatial:v1.2.8
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.2.8 --version
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.2.8 server --help
```
...
docker run --rm -i \
  ...
  ghcr.io/cafferychen777/chatspatial:v1.2.8 server --transport stdio
```
...
docker run --rm -p 8000:8000 \
  -v /absolute/path/to/your/data:/data:ro \
  -v /absolute/path/to/outputs:/outputs \
  ghcr.io/cafferychen777/chatspatial:v1.2.8 server --transport sse --host 0.0.0.0 --port 8000
```
...
Then replace `ghcr.io/cafferychen777/chatspatial:v1.2.8` with
...
| Pull fails | Check the image name and network access with `docker pull ghcr.io/cafferychen777/chatspatial:v1.2.8`. |
