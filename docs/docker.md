...
```text
ghcr.io/cafferychen777/chatspatial:v1.5.3
```

This guide uses the versioned `v1.5.3` tag so commands are reproducible. Use
...
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.5.3 --version
docker run --rm ghcr.io/cafferychen777/chatspatial:v1.5.3 server --help
```
...
docker run --rm -i \
  ...
  ghcr.io/cafferychen777/chatspatial:v1.5.3 server --transport stdio
```
...
docker run --rm -p 127.0.0.1:8000:8000 \
  -v /absolute/path/to/your/data:/data:ro \
  -v /absolute/path/to/outputs:/outputs \
  ghcr.io/cafferychen777/chatspatial:v1.5.3 server \
...
Then replace `ghcr.io/cafferychen777/chatspatial:v1.5.3` with
...
| Pull fails | Check the image name and network access with `docker pull ghcr.io/cafferychen777/chatspatial:v1.5.3`. |
